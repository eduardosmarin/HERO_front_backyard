...
Final_Order <- c("City","Census_Year","Census_Total_Population","Census_Households","Census_Ethnicity_Population",
  ...
  "Census_Median_Age","Census_Total_Population_Over_25", "Census_Median_Household_Income_Adjusted_2023",
...
All_Holyoke_Data_Y2017 <-
  ...
  mutate( #Doing weighted means based on each respective category
     Median_Household_Income_Adjusted_2023 =
      weighted.mean(Holyoke_CBG_Weighted_Y2017$Median_Household_Income_Adjusted_2023,
                    Holyoke_CBG_Weighted_Y2017$Households, na.rm = TRUE) 
  , Median_Gross_Rent_Adjusted_2023 =
      weighted.mean(Holyoke_CBG_Weighted_Y2017$Median_Gross_Rent_Adjusted_2023,
                    Holyoke_CBG_Weighted_Y2017$Renter_Occupied, na.rm = TRUE),
  , Median_Home_Value_Adjusted_2023 =
      weighted.mean(Holyoke_CBG_Weighted_Y2017$Median_Home_Value_Adjusted_2023,
                    Holyoke_CBG_Weighted_Y2017$Owner_Occupied, na.rm = TRUE)
  , Median_Age =
      weighted.mean(Holyoke_CBG_Weighted_Y2017$Median_Age,
                    Holyoke_CBG_Weighted_Y2017$Total_Population, na.rm = TRUE)
         ) |>
    rename_with(~ paste0("Census_", .x)) |>
...
All_Chelsea_Data_Y2017 <-
  ...
  mutate( #Doing weighted means based on each respective category
     Median_Household_Income_Adjusted_2023 =
      weighted.mean(Chelsea_CBG_Weighted_Y2017$Median_Household_Income_Adjusted_2023,
                    Chelsea_CBG_Weighted_Y2017$Households, na.rm = TRUE) 
  , Median_Gross_Rent_Adjusted_2023 =
      weighted.mean(Chelsea_CBG_Weighted_Y2017$Median_Gross_Rent_Adjusted_2023,
                    Chelsea_CBG_Weighted_Y2017$Renter_Occupied, na.rm = TRUE),
  , Median_Home_Value_Adjusted_2023 =
      weighted.mean(Chelsea_CBG_Weighted_Y2017$Median_Home_Value_Adjusted_2023,
                    Chelsea_CBG_Weighted_Y2017$Owner_Occupied, na.rm = TRUE)
  , Median_Age =
      weighted.mean(Chelsea_CBG_Weighted_Y2017$Median_Age,
                    Chelsea_CBG_Weighted_Y2017$Total_Population, na.rm = TRUE)
         ) |>
    rename_with(~ paste0("Census_", .x)) |>
  mutate(Census_Year = 2017) |>
  mutate(City = "Chelsea") |>
...
# IMPORTANT: need to turn this off otherwise each time you run it, a new version will be saved out. This whole chunk should be turned off. Also consider 
# write_csv(paste0("input_data/holyoke_chelsea_general_demographic_", Sys.Date(), ".csv")) to dissambiguate files in the future.
# All_Cities_Data_Combined |>
...
All_Cities_Data_Combined <- 
  read_csv("input_data/holyoke_chelsea_general_demographic.csv")
...
# 3. Finding the population change of Holyoke, MA
  All_Cities_Data_Combined |>
  filter(City == "Chelsea") |>
  arrange(Census_Year) |>
  summarise(
...
# BRILIANT above. Here's what I would have done, or something like that.
All_Cities_Data_Combined |> 
  group_by(City, Census_Year) |> 
  summarise(Population = sum(Census_Total_Population)) |> 
  pivot_wider(id_cols = City, names_from = Census_Year, values_from = Population) |> 
  mutate(change = `2017` - `2023`) # or make as percentage
