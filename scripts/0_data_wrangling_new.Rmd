...
Chelsea_Property_Data_Y2019 <-
  ...
  tidylog::select(4, 13) |> # NAMES not numbers in the future here and elsewhere. A little dangerous. No need to change, but in the next project it owuld be safer and more stable to use names
...
Pre_Chelsea_Holyoke_Giveaways_Y2017_Y2024 <-
  ...
  mutate(Species = ifelse(Common_Name == "Red Oak","Quercus rubra", Species))
```
...
# 1. Creating the Tree Type table
## a) Loading in the table and making some alterations
Pre_Tree_Type_Table <- 
  read_excel("input_data/General/Tree_Type_Table.xlsx") |>
  rename(Species = Genus_Species) |>
  mutate(Species = str_trim(Species)) |> # This was because there were some species here that had a space afterwards
  mutate(Species = case_when(
     Species == "Cladrastis kentukea" ~ "Cladastis kentukea"
   , Species == "Crataegus" ~ "Crataegus spp."
   , Species == "Fagus Spp." ~ "Fagus sylvatica" #EM: The only Fagus spp. that we had
   , Species == "Gingko biloba" ~ "Ginkgo biloba"
   , Species == "Halesia tetraptera" ~ "Halesia monticola" #DHL: WARNING | This may need a double check since the only Halesia is Monticola but assuming not the same as tetraptera (could go into the species concern list below)
   , Species == "Magnolia acuminata" ~ "Magnolia virginiana" #DHL: WARNING | This also may need a double check since it is not the same; more of a placeholder for now (could go into the species concern list below)
   , Species == "Juniperus spp." ~ "Juniperus virginiana" #EM: Similar as before, the only one that they had
   , Species == "Malus spp." ~ "Malus spp"
   , Species == "Metasequoia spp." ~ "Metasequoia glyptostroboides" #EM: The only one that we had that was similar
   , Species == "Ostrya spp." ~ "Ostrya virginana" #EM: Similar case here
   , Species == "Oxydendrum arboreum" ~ "Oxydendron arboreum"
   , Species == "Pinus nigra" ~ "Pinus strobus" #DHL: WARNING | This also may need a double check since it is not the same; more of a placeholder for now (could go into the species concern list below)
   , Species == "Prunus spp." ~ "Prunus (species)"
   , Species == "Thuja occidentalis" ~ "Thuja plicata" #DHL: WARNING | This also may need a double check since it is not the same; more of a placeholder for now (could go into the species concern list below)
   , Species == "Zelkova spp." ~ "Zelkova serrata"
, TRUE ~ Species
  )) |>
  mutate(Tree_Type = case_when(
    Tree_Type == "O" ~ "Ornamental"
  , Tree_Type == "C" ~ "Conifer"
  , Tree_Type == "SD" ~"Shade"
  , TRUE ~ NA_character_
  )) |>
  tidylog::select(-Common_Name)

# SPECIES CONCERN LIST
# DHL: After checking the database, it seems that we do not have the following species compared to the list provided by Prof. Rogan:
...
### ii) Checking for unmatched types
Unmatched_Type_Species_V1 <- 
  Pre_Chelsea_Holyoke_Giveaways_Y2017_Y2024 |>
  distinct(Species) |>
  anti_join(
    Pre_Tree_Type_Table |> distinct(Species),
    by = "Species"
  ) |>
  arrange((Species))
...
Unmatched_Type_Species_V2 <- # This one is more important for us
  Pre_Tree_Type_Table |>
  distinct(Species) |>
  anti_join(
    Pre_Chelsea_Holyoke_Giveaways_Y2017_Y2024 |> distinct(Species),
    by = "Species"
  ) |>
  arrange((Species))

Unmatched_Type_Species_V2 |>
  pull(Species) |> 
  paste(collapse = ", ")

### iii) Making the final table
Tree_Type_Table <-
 Pre_Tree_Type_Table |>
 rename_with(~ paste0("Ecological_", .x)
            , .cols = -any_of("Species"))

# 2. Making the Broadleaf and Conifers table
...
# 3. Creating the Native Status Table
Native_Status_Table <- 
...
# 4. Creating the iTree Final Table
...
Pre_V3_Giveaway_Y2017_Y2024 <-
  ...
  mutate(
    ...
    left_join(Tree_Type_Table, by = "Species") |> # EM: Adding this but will need to change in the future
...
My_Vars <-
  ...
  , 'ed_Collexge_Less_1_Year'                = 'B15003_019'
