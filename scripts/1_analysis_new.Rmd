...
Final_Giveaway_Y2017_Y2024 <-
  st_read("input_data/Final_Giveaway_Y2017_Y2024_2026-03-25.gpkg") |>
  st_drop_geometry()
  ...
  filter(
    any(
      ...
      # Case 4: Dead in T2 & Date of the Planting > Inspection Date in T1 (Planted in between T1 and T2)
        Survival_Inspection == 2 &
     !is.na(Survival_Inspection_Date) &
     Date_Planted > as_date("2017-06-27")
      , na.rm = TRUE)
  ) |>
...
Chi_Giveaway_Y2017_Y2024 |>
  tabyl(Status, City) |> 
  adorn_totals("row") # more valid obs in Holyoke than Chelsea
...
Chi_Giveaway_Y2017_Y2024 |>
  ...
  adorn_totals("row") |>
  adorn_percentages("col") |>
  adorn_pct_formatting(digits = 1) # similar survivorship, a little higher in Chelsea

# 3. City and Survivorship (Chi Square)
Chi_Giveaway_Y2017_Y2024 |>
  janitor::tabyl(Status, City) |> 
  janitor::chisq.test() |> 
  broom::tidy() # and that difference is not significant
```
...
Chi_Giveaway_Y2017_Y2024 |>
  count(Time, City, Status) |> # slick! skips the group_by, very nice
  pivot_wider(
     names_from = Status
   , values_from = n
  ) |>
...
Chi_Giveaway_Y2017_Y2024 |> 
  ...
  tidyr::pivot_wider(
    names_from = Status
  , values_from = n
  , values_fill = 0 # instead of the ifelse in a mutate, great job
  ) |>
  mutate(
     Total = Alive + Dead
...
bind_rows(
  ...
  , Chi_Giveaway_Y2017_Y2024 |>
      ...
      janitor::chisq.test() |>
      broom::tidy()
  , .id = "T" # not so helpful here, but very useful in other bind_row applications.
    ) 
```
...
Chi_Giveaway_Yard_Y2017_Y2024 |>
  ...
  adorn_totals("row") |>
  adorn_percentages("col") |>
  adorn_pct_formatting(digits = 1) # What is your interpretation of this?
...
### ii) City Survivorship DHL: I think this is a repeat from '### ii) City Survivorship above?'
```{r}
# 1. City and Survivorship (Count)
Chi_Giveaway_Yard_Y2017_Y2024 |>
  tabyl(Status, City) |> 
...
Chi_Giveaway_Yard_Y2017_Y2024 |>
  ...
  janitor::chisq.test() |> 
  broom::tidy() # NOT significant, but this test is naive to time in ground AND potential confounders
...
# DHL to ESM: question for John/Adrian?, or should we scrutinze the prior processing steps?
...
Chi_Giveaway_Yard_Y2017_Y2024 |> 
  ...
  arrange(Yard, Time) # Back is more lickely to be dead than front or side
...
Overall_Survival_Curve_Data <-
  General_Giveaway_Y2017_Y2024 |>
  mutate(Years_In_Ground = Survival_Days_In_Ground / 365.25) |>
  mutate(
     Time = as.numeric(Years_In_Ground) # Trying to ignore the NAs
...
# there should be two rows for each tree, one for time 1 and one for time 2
# Survival_Days_In_Ground should be greater in time 2 than time 1
Overall_Survival_Curve_Data |> 
  select(Unique_ID : Time_Period, Time, Event, Survival_Days_In_Ground) |> 
  filter(Unique_ID == 'C995') |> 
  glimpse()
...
DHL to ESM: what you have above was to gain an intuitive sense for these data and to double checks. What you have right below could be suitable for the paper and/or presentations. They both have value
...
ggsurvfit::ggsurvfit(Overall_KM_Model) +
  ...
  scale_x_continuous(
    name = "Time (Years)",
    breaks = seq(0, 4000, by = 365*1) / 365.25, 
    labels = round(seq(0, 4000, by = 365*1) / 365.25, 1)
  ) +
  labs(
    y = "Survival Probability",
    title = "Kaplan–Meier Curve for All GCC Tree Plantings",
    subtitle = "Holyoke and Chelsea, MA — HERO Program 2017 & 2024 Surveys"
  )

# DHL to ESM, after 4000 days there is just 1 tree alive and 991 have died?
...
# DHL to ESM this has much more survivorship than street trees in Chelsea
...
# DHL to ESM What's up witih NA's? Fix or cut
...
# 9. Cross tabs for Tree Type by City
## a) Count & Percentage
Residential_Survival_Curve_Data |> # EM: Doesn't seem to have enough statistical power; we might need more of them
  janitor::tabyl(Ecological_Tree_Type, City) |>
  janitor::adorn_totals("row") |>
  janitor::adorn_percentages("col") |>
  janitor::adorn_pct_formatting(digits = 1) |>
  janitor::adorn_ns()
...
# Ecology, Should Use Alone? Yes
Residential_Survival_Curve_Data |> #  EM: Doesn't seem to have enough statistical power; we might need more of them; Very few trees to be able to identifiy this
  janitor::tabyl(Ecological_Tree_Type, City, Yard) |>
  janitor::adorn_totals("row") |>
  janitor::adorn_percentages("col") |>
  janitor::adorn_pct_formatting(digits = 1) |>
  janitor::adorn_ns()
...
# DHL to ESM almost everything dies by 4000 days?
...
#### E) Tree Type [Include: Yes!—although low statistical power relatively, it seems to be promising]
##### 1. Overall
```{r}
# 1. Creating [Tree Type] Kaplan-Meier Survivorship Curve
## a) Creating the model
Overall_Tree_Type_KM_Model <- 
  ggsurvfit::survfit2(Surv(Time, Event) ~ Ecological_Tree_Type, data = Overall_Survival_Curve_Data) 

## b) Visualizing the model
ggsurvfit::ggsurvfit(Overall_Tree_Type_KM_Model) +
  ggsurvfit::add_confidence_interval() +
  ggsurvfit::add_risktable() +
  ggsurvfit::add_censor_mark() +
   labs(
     x = "Time (Expressed in Days)"
   , y = "Survival probability"
   , title = "Tree Type Kaplan-Meier Curve for All Tree Plantings from Holyoke, MA and Chelsea, MA"
   , subtitle = "As part of the HERO Program at Clark University, in 2017 and 2024, students surveyed tree mortality \nconcerning giveaways implemented through the Greening the Gateway Cities Program" 
  )

## c) Testing the strength of the model
survdiff(Surv(Time, Event) ~ Ecological_Tree_Type, data = Overall_Survival_Curve_Data)
# P-value: 0.1 | Significant: No #Even with little number, this seems very promisign; although we need to make sure the species are correct here

# 2. Creating [Tree Type x City] Kaplan-Meier Survivorship Curve
## a) Creating the model
Overall_Tree_Type_City_KM_Model <- 
  ggsurvfit::survfit2(Surv(Time, Event) ~ Ecological_Tree_Type + City, data = Overall_Survival_Curve_Data) 

## b) Visualizing the model
ggsurvfit::ggsurvfit(Overall_Tree_Type_City_KM_Model) +
  ggsurvfit::add_confidence_interval() +
  ggsurvfit::add_risktable() +
  ggsurvfit::add_censor_mark() +
   labs(
     x = "Time (Expressed in Days)"
   , y = "Survival probability"
   , title = "Tree Type and City-Based Kaplan-Meier Curve for All Tree Plantings from Holyoke, MA and Chelsea, MA"
   , subtitle = "As part of the HERO Program at Clark University, in 2017 and 2024, students surveyed tree mortality \nconcerning giveaways implemented through the Greening the Gateway Cities Program" 
  )

## c) Testing the strength of the model
survdiff(Surv(Time, Event) ~ Ecological_Tree_Type + City, data = Overall_Survival_Curve_Data)
# P-value: 3e-11 | Significant: Yes
```
...
# DHL to ESM: Large (street trees from prior analyses) in Chelsea seem to do the worst. Does that match prior HERO work and your work above?
```
...
# DHL to ESM: if you made a categorical variable from a continuous variable, just to look at it in K-M, and you want to keep it in the Cox PH, use the continuous version in the the coxph. So Parcel_Total_Value below, not Parcel_Total_Value_Group. In generall.
...
Baseline_Residential_CPH_Model <-
  survival::coxph(
...
ls() # everything in memory, pattern for filitering
# DHL to ESM ls() lists everything in memory, the pattern argument filters to only objects with that name see help(ls)
...
Yard_Residential_Add_CPH_Model <-
  survival::coxph(
    Surv(Time, Event) ~ City + Yard
      , data = Residential_Survival_Curve_Data
    )

Yard_Residential_Int_CPH_Model <-
  survival::coxph(
    Surv(Time, Event) ~ City*Yard
      , data = Residential_Survival_Curve_Data
    )
...
Yard_Residential_Int_CPH_Model |> broom::tidy()
Yard_Residential_Int_CPH_Model |> ggeffects::ggpredict()

# 4. Plotting the values
Yard_Residential_Int_CPH_Model |> ggeffects::ggpredict() |> plot()
...
```{r}
# 1. Checking each of the variables
## a) Season Planted (SP)
Ecological_Residential_SP_CPH_Model <- 
  survival::coxph(
    Surv(Time, Event) ~ Season_Planted
    , data = Residential_Survival_Curve_Data
  )

## b) Native Status (NS)
Ecological_Residential_NS_CPH_Model <- 
  survival::coxph(
...
## c) Growth Rate (GR)
Ecological_Residential_GR_CPH_Model <- 
  survival::coxph(
    Surv(Time, Event) ~ Ecological_Native_Status
    , data = Residential_Survival_Curve_Data
  )
...
## d) Longevity 
Ecological_Residential_Longevity_CPH_Model <- 
  survival::coxph(
    Surv(Time, Event) ~ Ecological_Longevity
    , data = Residential_Survival_Curve_Data
  )

## d) Size Classes (Height Maturity [HM])
Ecological_Residential_HM_CPH_Model <- 
  survival::coxph(
    Surv(Time, Event) ~ Ecological_Height_Maturity
    , data = Residential_Survival_Curve_Data
  )

## e) Tree Type
Ecological_Residential_Tree_Type_CPH_Model <- 
  survival::coxph(
    Surv(Time, Event) ~ Ecological_Tree_Type
    , data = Residential_Survival_Curve_Data
  ) # seems to be insigificant
...
, Ecological_Residential_Longevity_CPH_Model
, Ecological_Residential_HM_CPH_Model
, Ecological_Residential_Tree_Type_CPH_Model # Tree Type is preferred more actually!
, rank = TRUE)

# 3. Results
## Based off of the AIC weight, 
### - Tree Type
### - Longevity
### - Tree Height at Maturity (goes down later)
## should be used as they are the best predictors.
```

### ii) Full Selected Model
```{r}
# 1. Creating the Ecological Baseline Model
Ecological_Residential_CPH_Model <-
  survival::coxph(
    Surv(Time, Event) ~ City + Ecological_Longevity + Ecological_Height_Maturity
    , data = Residential_Survival_Curve_Data
    )

# 2. Extracting the Values
Ecological_Residential_CPH_Model |> broom::tidy()
Ecological_Residential_CPH_Model |> ggeffects::ggpredict()

# 3. Plotting the variables
## a) Calculating the values for the continuous variables
quantile(Residential_Survival_Curve_Data$Ecological_Height_Maturity
       , probs = c(0, 0.25, 0.5, 0.75, 1)
       , na.rm = TRUE)

Ecological_Height_Maturity_Dist_Value <-
  quantile(Residential_Survival_Curve_Data$Ecological_Height_Maturity
         , probs = c(0, 0.25, 0.5, 0.75, 1)
         , na.rm = TRUE) |>
  round(2)

Ecological_Height_Maturity_Term <-
  paste0("Ecological_Height_Maturity ["
       , paste(Ecological_Height_Maturity_Dist_Value, collapse = ", ")
       , "]")

## b) Making the ecological models 
Ecological_Height_Maturity_Predictors <-
  ggeffects::ggpredict(
    Ecological_Residential_CPH_Model
  , terms = Ecological_Height_Maturity_Term
  ) |>
  mutate(variable = "Ecological_Height_Maturity") |>
  arrange(x) |>
  dplyr::mutate(
    x = factor(x, levels = sort(unique(x)))  
  )

Ecological_Longevity_Predictors <-
  ggeffects::ggpredict(
    Ecological_Residential_CPH_Model
  , terms = "Ecological_Longevity"
  ) |>
  mutate(variable = "Ecological_Longevity"
   , x = factor(x, levels = c("Short", "Moderate", "Long"))  
  )

### c) Combining the ecological models
Ecological_Model <-
  bind_rows(
     Ecological_Height_Maturity_Predictors
   , Ecological_Longevity_Predictors
...
# 1. Checking each of the variables
## a) Lot Size (LS)
Parcel_Residential_LS_CPH_Model <- 
  survival::coxph(
    Surv(Time, Event) ~ Parcel_Lot_Size
    , data = Residential_Survival_Curve_Data
  )

## b) Age [Year Built (YB)]
Parcel_Residential_YB_CPH_Model <- 
  survival::coxph(
    Surv(Time, Event) ~ Parcel_Year_Built
    , data = Residential_Survival_Curve_Data
  )

## c) Total Value (TV)
Parcel_Residential_TV_CPH_Model <- 
  survival::coxph(
    Surv(Time, Event) ~ Parcel_Total_Value
    , data = Residential_Survival_Curve_Data
  )
...
```{r}
# 1. Creating the Ecological Baseline Model
Parcel_Residential_CPH_Model <-
  survival::coxph(
    Surv(Time, Event) ~ City + Parcel_Lot_Size
    , data = Residential_Survival_Curve_Data
    )

# 2. Extracting the Values
Parcel_Residential_CPH_Model |> broom::tidy()
Parcel_Residential_CPH_Model |> ggeffects::ggpredict()

# 3. Plotting the variables
## a) Calculating the values for the continuous variables
quantile(Residential_Survival_Curve_Data$Parcel_Lot_Size
       , probs = c(0, 0.25, 0.5, 0.75, 1)
       , na.rm = TRUE)

Parcel_Lot_Size_Dist_Value <-
  quantile(Residential_Survival_Curve_Data$Parcel_Lot_Size
         , probs = c(0, 0.25, 0.5, 0.75, 1)
         , na.rm = TRUE) |>
  round(2)

Parcel_Lot_Size_Term <-
  paste0("Parcel_Lot_Size ["
       , paste(Parcel_Lot_Size_Dist_Value, collapse = ", ")
       , "]")
...
Parcel_Model <-
  ggeffects::ggpredict(
    Parcel_Residential_CPH_Model
  , terms = Parcel_Lot_Size_Term
  ) |>
  mutate(variable = "Parcel_Lot_Size") |>
  arrange(x) |>
  dplyr::mutate(
    x = factor(x, levels = sort(unique(x)))  
  ) |>
...
Parcel_Model |>
  ...
  labs(
     x = "Predictor Level"
  ,  y = "Predicted Survival (Relative Hazards/Risks)"
  ,  title = "Predicted Survival by Parcel Factors"
  ) +
  theme_minimal(base_size = 14) +
  NULL
...
Census_Residential_MHI_CPH_Model <- 
  survival::coxph(
    Surv(Time, Event) ~ Census_Median_Household_Income
    , data = Residential_Survival_Curve_Data
  )

## b) Census Percentage At Least High School (ALHS)
Census_Residential_ALHS_CPH_Model <- 
  survival::coxph(
    Surv(Time, Event) ~ Census_Percentage_At_Least_High_School
    , data = Residential_Survival_Curve_Data
  )

## c) Census Percentage At Least College (ALC)
Census_Residential_ALC_CPH_Model <- 
  survival::coxph(
    Surv(Time, Event) ~ Census_Percentage_At_Least_College
  , data = Residential_Survival_Curve_Data
  )

## d) Census White Alone (WA)
Census_Residential_WA_CPH_Model <- 
  survival::coxph(
    Surv(Time, Event) ~ Census_White_Alone_Percentage
    , data = Residential_Survival_Curve_Data
  )

## e) Census Below Poverty (BP)
Census_Residential_BP_CPH_Model <- 
  survival::coxph(
    Surv(Time, Event) ~ Census_Below_Poverty_Percentage
    , data = Residential_Survival_Curve_Data
  )

## f) Census Unemployment 
Census_Residential_Unemployment_CPH_Model <- 
  survival::coxph(
    Surv(Time, Event) ~ Census_Unemployed_Percentage
    , data = Residential_Survival_Curve_Data
  )

## g) Census Owner Occupied (OO)  
Census_Residential_OO_CPH_Model <- 
  survival::coxph(
    Surv(Time, Event) ~ Census_Owner_Occupied_Percentage
    , data = Residential_Survival_Curve_Data
  )

# 2. Census Comparing Residential
performance::compare_performance(
 Census_Residential_MHI_CPH_Model
, Census_Residential_ALHS_CPH_Model
, Census_Residential_ALC_CPH_Model
...
### ii) Full Selected Model
```{r}
# 1. Creating the Census Baseline Model
Census_Residential_CPH_Model <-
  survival::coxph(
     Surv(Time, Event) ~ City + Census_Median_Household_Income
   , data = Residential_Survival_Curve_Data
  )

# 2. Extracting the values
Census_Residential_CPH_Model |> broom::tidy()
Census_Residential_CPH_Model |> ggeffects::ggpredict()

# 3. Plotting the variables
## a) Calculating the values for the continuous variable
Census_Median_Household_Income_Dist_Value <-
  quantile(
     Residential_Survival_Curve_Data$Census_Median_Household_Income
   , probs = c(0, 0.25, 0.5, 0.75, 1)
   , na.rm = TRUE
  ) |>
  round(-3)
...
## b) Making the model of Median Household Income (MHI) 
Census_Model <-
  ggeffects::ggpredict(
    Census_Residential_CPH_Model
  , terms = Census_Median_Household_Income_Term
  ) |>
  dplyr::mutate(variable = "Census_Median_Household_Income") |>
...
Census_Model |>
  ...
  labs(
   ...
   , y = "Predicted Relative Hazard"
   , title = "Predicted Hazard by Census Median Household Income"
  ) +
...
### i) Comparing each individual model
```{r}
# 1. Comparing each individual model (w/out Yard and City for comparison)
performance::compare_performance(
  Ecological_Residential_SP_CPH_Model
, Ecological_Residential_NS_CPH_Model
, Ecological_Residential_GR_CPH_Model
...
# Results: Ranking of each of the models are the following:
# 1. Ecological_Residential_Tree_Type_CPH_Model     65.22% #DHL: States its the best mosdel currently
# 2. Ecological_Residential_Longevity_CPH_Model     40.00%
# 3. Census_Residential_ALHS_CPH_Model              25.97%
# 4. Census_Residential_ALC_CPH_Model               21.24%
# 5. Census_Residential_Unemployment_CPH_Model      21.09%
# 6. Ecological_Residential_NS_CPH_Model            19.53%
# 7. Ecological_Residential_GR_CPH_Model            19.53%
# 8. Parcel_Residential_LS_CPH_Model                17.48%
# 9. Ecological_Residential_HM_CPH_Model            16.86%
# 10. Census_Residential_WA_CPH_Model               15.25%
# 11. Census_Residential_BP_CPH_Model               12.84%
# 12. Parcel_Residential_TV_CPH_Model               11.79%
# 13. Census_Residential_MHI_CPH_Model              11.26%
# 14. Ecological_Residential_SP_CPH_Model           10.33%
# 15. Census_Residential_OO_CPH_Model               9.44%
# 16. Parcel_Residential_YB_CPH_Model               9.25%
  
# 2. Comparing each individual model (w Yard and City for comparison)
performance::compare_performance(
  Baseline_Residential_CPH_Model
, Yard_Residential_Int_CPH_Model
, Ecological_Residential_SP_CPH_Model
...
# Results: Ranking of each of the models (including Yard and City for comparison)
# 1. Ecological_Residential_Tree_Type_CPH_Model 62.54% # DHL: Again, it seems to be saying it is the better model
# 2. Yard_Residential_Int_CPH_Model             32.83%
# 3. Baseline_Residential_CPH_Model             31.77%
# 4. Ecological_Residential_Longevity_CPH_Model 25.38%
# 5. Census_Residential_ALHS_CPH_Model          15.79%
# 6. Census_Residential_Unemployment_CPH_Model  13.49%
# 7. Census_Residential_WA_CPH_Model            12.00%
# 8. Census_Residential_ALC_CPH_Model           11.71%
# 9. Parcel_Residential_LS_CPH_Model            10.90%
# 10. Parcel_Residential_TV_CPH_Model            10.52%
# 11. Ecological_Residential_HM_CPH_Model        10.32%
# 12. Ecological_Residential_SP_CPH_Model         9.83%
# 13. Census_Residential_OO_CPH_Model             9.44%
# 14. Parcel_Residential_YB_CPH_Model             9.24%
# 15. Census_Residential_BP_CPH_Model             8.51%
# 16. Census_Residential_MHI_CPH_Model            5.91%
# 17. Ecological_Residential_NS_CPH_Model         5.26%
# 18. Ecological_Residential_GR_CPH_Model         5.26%
```

### ii) Comparing each full selected model
```{r}
...
# ESM to DHL: The best model seems to be the ecoloigcal still, primarily due to the inclusion of longevity. Makes sense: trees with longer longevity have less deaths compared to others. Let me know what else I should do or if this has been exhausted!

maximal_model <- 
  survival::coxph(
    Surv(Time, Event) ~ City*Yard + Ecological_Tree_Type + Ecological_Height_Maturity + Census_Median_Household_Income + Parcel_Lot_Size
    , data = Residential_Survival_Curve_Data
  )
...
maximal_model_plots$City
maximal_model_plots$Ecological_Tree_Type #DHL: This doesn't make much sense; none are signifiant—the largest takeaway was that MHI is th best indiciator
maximal_model_plots$Ecological_Height_Maturity
maximal_model_plots$Parcel_Lot_Size

maximal_model |> 
  ggeffects::ggpredict(terms = c("City", "Yard")) |> 
  plot()
...
```{r}
survival::coxph(Surv(Time, Event) ~ City+Yard + Ecological_Longevity, data = Residential_Survival_Curve_Data)
survival::coxph(Surv(Time, Event) ~ City*Yard + Ecological_Longevity, data = Residential_Survival_Curve_Data)

survival::coxph(Surv(Time, Event) ~ City+Yard + Parcel_Residential_Area, data = Residential_Survival_Curve_Data)
survival::coxph(Surv(Time, Event) ~ City*Yard + Parcel_Residential_Area, data = Residential_Survival_Curve_Data)

survival::coxph(Surv(Time, Event) ~ City+Yard + Census_Percentage_At_Least_High_School, data = Residential_Survival_Curve_Data)
survival::coxph(Surv(Time, Event) ~ City*Yard + Census_Percentage_At_Least_High_School, data = Residential_Survival_Curve_Data)


one <- survival::coxph(Surv(Time, Event) ~ City+Yard + Ecological_Longevity + Parcel_Residential_Area + Census_Percentage_At_Least_High_School, data = Residential_Survival_Curve_Data)
two <- survival::coxph(Surv(Time, Event) ~ City*Yard + Ecological_Longevity + Parcel_Residential_Area + Census_Percentage_At_Least_High_School, data = Residential_Survival_Curve_Data)

performance::compare_performance(one, two, rank = TRUE)

AIC(one)
AIC(two)

p <- two |> ggeffects::ggpredict() |> plot()

p$City
p$Yard
p$Ecological_Longevity
p$Parcel_Residential_Area
p$Census_Percentage_At_Least_High_School

p_int <- two |> ggeffects::ggpredict(terms = c('City', 'Yard')) |> plot() + geom_hline(yintercept = 1)
p_int

p_int_all <- two |> ggeffects::ggpredict() |> plot() + geom_hline(yintercept = 1)
p_int_all$City
p_int_all$Yard
p_int_all$Ecological_Longevity
p_int_all$Parcel_Residential_Area
p_int_all$Census_Percentage_At_Least_High_School


# gt
# gtsummary
# something to make the output publication-ready
two |> broom::tidy(conf.int = TRUE)

two |> ggeffects::ggpredict(terms = c('City', 'Yard'))
