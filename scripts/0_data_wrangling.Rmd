...
Chelsea_Property_Data_Append_Y2019 <-
  ...
  tidylog::select(Parcel_Unique_ID, Parcel_Property_ID, Parcel_Location_ID, Parcel_Lot_Size, Parcel_Lot_Date, Parcel_Lot_Units, Parcel_Lot_Price_Adjusted_2023, Parcel_Building_Value_Adjusted_2023, Parcel_Land_Value_Adjusted_2023, Parcel_Other_Value_Adjusted_2023, Parcel_Total_Value_Adjusted_2023, Parcel_Year, Parcel_Style, Parcel_Use_Code, Parcel_Zoning, Parcel_Year_Built, Parcel_Building_Area, Parcel_Residential_Area, Parcel_Units, Parcel_Number_Rooms, Parcel_Stories, Time_Period, Parcel_Year_Dataset)
        ...
        , General_Location = case_when(
         ...
         mutate(
          General_Site_Type = case_when(
            Site_Type %in% c("Sidewalk Cutout", "Sidewalk Planting Strip") ~ "Street Trees"
          , Site_Type %in% c("Back Yard", "Side Yard", "Front Yard") ~ "Yard Trees"
          , Site_Type %in% c("Other Maintained", "Maintained Park") ~ "Maintained Area"
          , TRUE ~ NA_character_)
...
## a) Double checking missignness with mortality
```{r}
Final_Giveaway_Y2017_Y2024 |> 
...
Massachusetts_CBG_Data_Y2017 <-
  tidycensus::get_acs(
    ...
    tidylog::select(-tidylog::ends_with('M')) |>
    tidylog::rename_with(~ gsub('E$', '', .x), tidylog::ends_with('E')) |>
    rename(NAME = NAM) |>
    tidyr::separate( 
        NAME
    ...
    tidylog::mutate(
      Total_Population_Over_25 = tidylog::na_if(Total_Population_Over_25, 0)
    , Percentage_At_Least_High_School =
        rowSums(tidylog::across(tidylog::starts_with('ed_')), na.rm = TRUE) /
        Total_Population_Over_25
    , Percentage_At_Least_College =
        rowSums(tidylog::across(tidylog::starts_with('ed_plus')), na.rm = TRUE) /
        Total_Population_Over_25
    ) |>
    ...
    tidylog::select(-tidylog::starts_with('ed_')) |>
