...
Final_Giveaway_Y2017_Y2024 <-
  st_read("input_data/Final_Giveaway_Y2017_Y2024.gpkg") |>
  st_drop_geometry()
  
# 2. Developing the code generally used for the survivorship study 
General_FY_BY_Giveaway_Y2017_Y2024 <-
  Final_Giveaway_Y2017_Y2024 |>
  filter(Site_Type %in% c("Front Yard", "Backyard")) |>
  # dplyr::select(-starts_with("Census"), -starts_with("Parcel")) |> #Why I gave them the prefixes earlier :)
  group_by(Identifier) |>
  filter(
    any(
      # Case 1: Tree observed at both Time 1 and Time 2
      Survival_Has_T1_Observation & Survival_Has_T2_Observation | #or
      # Case 2: Seen in T1 and dead at T1
      (Survival_Has_T1_Observation & Survival_Status == 1 & Survival_Inspection == 1) | #or
      # Case 3: Seen in T2 (alive, so alive before)
      (Survival_Status == 0 & Survival_Inspection == 2)
    , na.rm = TRUE)
  ) |>
  ungroup()
...
## b) Preparing the data for the Chi Square
Chi_Fisher_Giveaway_Y2017_Y2024 <-
 ...
 dplyr::select(Identifier, City, Yard, Time, Status) |>
...
# 2. Creating the table for percentages | Time and survivorship
Chi_Fisher_Giveaway_Y2017_Y2024 |>
  tabyl(Time, Status) |>
  adorn_totals("row") |>
...
# 4. Takeaways
# - The p-value is very high (p =.7522). The issue is that there is very small amount of additional deaths in T2. This may lead to why the relationship is not statistically significant. The above will likely be a continued issue going forward.
```
...
Chi_Fisher_Giveaway_Y2017_Y2024 |>
  ...
  dplyr::select(Time, Yard, Total, `Alive (%)`, `Dead (%)`)
...
Chi_Fisher_Giveaway_Y2017_Y2024 |>
  ...
  dplyr::select(Time, City, Total, `Alive (%)`, `Dead (%)`)
...
Chi_Fisher_Giveaway_Y2017_Y2024 |> 
  ...
  dplyr::select(Time, City, Yard, Total, `Alive (%)`, `Dead (%)`) |>
  arrange(Time, City, Yard)
...
ggsurvfit::ggsurvfit(Giveaway_Kaplan_Model) +
