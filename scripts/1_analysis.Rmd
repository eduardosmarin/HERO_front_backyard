...
-   Learned this after getting some warnings from my chi squares when
    sample size was too small. When cell counts are lower than five, we
    ought to use the Fisher test. However, when cell counts are higher
    than five, chi squares become more ideal:
    <https://www.ccjm.org/content/84/9_suppl_2/e20> &
    [https://medium.com/\@alb.formaggio/chi-square-and-fishers-test-does-it-really-matter-614a8cb937af](https://medium.com/@alb.formaggio/chi-square-and-fishers-test-does-it-really-matter-614a8cb937af){.uri}
...
Yard_Survival_Curve_Data <-
  ...
  tidylog::select(-starts_with("Census_")) |> #Like you mentioned earlier, now we have the option to select and deselect from these exact variables | we can explore within each category
  tidylog::select(-starts_with("Ecological_")) |>
  tidylog::select(-starts_with("Parcel_"))
...
General_Giveaway_Y2017_Y2024 |> filter(Identifier == 'CHELSEA_352') #ESM: Seems to work now, also did my double checks for some clarity
# as an example yield two rows.

General_Giveaway_Y2017_Y2024 |> glimpse()
General_Giveaway_Y2017_Y2024 |> 
  select(Survival_Days_In_Ground, Identifier : Date_Surveyed, starts_with('Survival'))
...
Overall_Giveaway_Kaplan_Model <- 
  ggsurvfit::survfit2(Surv(Time, Event) ~ 1, data = Yard_Survival_Curve_Data) # tried piping but didn't work
...
City_Giveaway_Kaplan_Model <- 
  ggsurvfit::survfit2(Surv(Time, Event) ~ City, data = Yard_Survival_Curve_Data) 
...
Yard_Giveaway_Kaplan_Model <- 
  ggsurvfit::survfit2(Surv(Time, Event) ~ Yard, data = Yard_Survival_Curve_Data) 
...
## a) City
survdiff(Surv(Time, Event) ~ City, data = Yard_Survival_Curve_Data) # Read the package for survival and it says survdiff "Tests if there is a difference between two or more survival curves using the  G ρ G  ρ family of tests, or for a single curve against a known alternative"—not sure what they mean, however

## b) Yard
survdiff(Surv(Time, Event) ~ Yard, data = Yard_Survival_Curve_Data) # Read the package for survival and it says survdiff "Tests if there is a difference between two or more survival curves using the  G ρ G  ρ family of tests, or for a single curve against a known alternative"—not sure what they mean, however

## c) Yard + City
survdiff(Surv(Time, Event) ~ Yard + City, data = Yard_Survival_Curve_Data) # Again, very low..
...
General_Location_Survival_Curve_Data <-
  ...
  tidylog::select(-starts_with("Census_")) |> 
  tidylog::select(-starts_with("Ecological_")) |>
  tidylog::select(-starts_with("Parcel_"))
...
End takeways (V2): - Location does seem to matter for tree survivorship,
more generally if it is in a residential vs non-residential space.
However, specific locations within a residential area may not lead to—at
least from this dataset—any significance. As you can see, even including
right-of-way, the p-value (\<0.07) and tht there isn't ny experienced
difference in survivorship between Front, Side, Back, and Right-of-way

End takeaways: - This is a bit of a yikes since the data set was overall
to small, specifically for the "landscape mullets" concept to see if
there was any discernible trend. I did enjoy the data analysis part but
had challenges overall with grappling with what this mean (which feels
like not much) - For following steps for thesis, I might want to step a
bit more broadly. Instead of limiting the dataset to just the
observations in FY BY, I can see if generally survivorship (which these
curves are cool but need more fleshing out) are generally impacted by
factors like property parcel value, education, etc. Maybe that could be
the next thing I complete this week so I can start writing. - Currently,
I belive I am unable to ask questions concerning income and landscape
mullet just due to the size of the dataset.
...
mod_res <- 
  survival::coxph(
    Surv(Time, Event) ~ Residential 
    , data = dat
  )

mod_city_res_add <- 
  survival::coxph(
    Surv(Time, Event) ~ City + Residential
    , data = dat
  )
...
mod_city_res_int_parcel_yr <- 
  survival::coxph(
    Surv(Time, Event) ~ City*Residential + Parcel_Year
    , data = dat
  )
...
# select_and_plot_mod <- function(mod_1, mod_2, mod_3, mod_4) {
#   winning_mod <- 
#     compare_performance(
#         mod_1
#       , mod_2
